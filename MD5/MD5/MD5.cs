...
namespace MD5
{
    public class MD5
    {
        ...
        public string Value
        {
            ...
            set
            {
                A = 0x67452301;//первичные значения 
                B = 0xEFCDAB89;
                C = 0x98BADCFE;
                D = 0X10325476;
        ...
        void Create512()
        {
            ...
            if (dopSize != 0)
            {
                ...
                bt[0] = 0x80;//создание нового массива из 1 и много 0
                for (int i = 1; i < bt.Length; i++) bt[i] = 0;
                input = input.Concat( bt ).ToArray();
            }

            input = input.Concat( BitConverter.GetBytes( currentSize - 64 ) ).ToArray();//слияние строк
        }
      ...
      /*  /// <summary>
        ...
        /// <returns></returns>*/
        public static uint RotateLeft( uint uiNumber, ushort shift )   { return ( ( uiNumber >> (32 - shift) ) | ( uiNumber << shift ) ); }
